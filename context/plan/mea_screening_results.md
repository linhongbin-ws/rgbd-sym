...
## 8. 修复后 stage-1 结果(2026-07-18):V2FIX+REFL 仍不敌 baseline → 干净的冗余+稀释证据

3 × V2FIX+REFL 跑完(wandb 完整曲线 AUC;BASE 复用 §2-3 的 equi run,seed 配对):

| arm | s0 | s1 | s2 | **mean** |
|---|---|---|---|---|
| BASE | 0.323 | 0.353 | 0.487 | **0.388** |
| V2FIX+REFL | 0.278 | 0.246 | 0.209 | **0.244** |
| **配对差** | −0.045 | −0.107 | −0.278 | **−0.143(0/3)** |

参考:V1 0.320;旧错标签 V2+REFL 0.337、V2ROT 0.223。**修复标签并没有救回增强臂**(0.244 vs 错标签版 0.337,差异在 seed 噪声内)→ 对 +REFL 臂,标签错误从来不是主约束;主约束是**冗余+稀释**(见下)。s2 的 −0.278 主要由 BASE s2 outlier(0.487)拉开。

> ⚠️ 首次 analyze 时默认 tag `d15_s` 把 normal 网 run(`nrm_d15_s` 含该子串)混进了 BASE/V2+REFL 桶,污染对照行——analyzer 已修(默认 `--exclude nrm_`,commit 9468d60)。上表为手工按 run 名清分后的干净数。

### 机理(现在是干净结论)

增强能提供的三种内容,网络/管线已各自拥有:①90° 旋转 = C4 等变**结构内建**;②平移 = 卷积内建;③连续中间角 ≈ 等变网络自动插值,且 `seq_rot` buffer 本来就对两臂都做训练时旋转增强。**唯一新内容 = 反射(×2 因子)**,来自同样的 15 个场景,信息量微小。而代价实打实:专家池 92% 是同 15 个场景的克隆 → replay 分布向零新信息的克隆数据**加权 13×**(稀释根因 D)。收益≈0、成本>0 → 略低于 baseline。

- stage 2(V2FIX-ROT 消融)进行中;s0 中途 AUC 平 0(纯旋转克隆 = 100% 冗余 = 纯稀释,若终值仍垫底,与本机理自洽)。
- **下一个信息量最大的实验:normal 网 + 修复标签**(`bash/bench_normal.sh` 已更新为只跑 `v2fgr_nrm_` 臂,BASE-normal 复用,3 run ≈ 1 天)——非等变网**没有**内建对称,若修复版增强在它身上也不帮忙,则「增强注入的对称先验有价值」整个前提都要重估;若帮忙,则「增强=用样本换架构对称」的论文叙事成立。
