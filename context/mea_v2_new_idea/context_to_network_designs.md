...
## 1b. ⚠️ 2026-07-27 重大更正:桌面任务在所有相位都是 SE(2),没有相位退化

经用户连续两问证伪了"接触相位群退化"的前提(对**当前桌面任务**):

- **绕 z(yaw)在接触相位仍成立**——整场景绕竖直轴旋转物理合法、reward 不变;夹爪居中观测下 C4 网的等变就等于全局绕 z 旋转 → **C4 在接触处不破缺**(不是我 §1/§2 早先说的那样)。
- **pitch/roll 在 approach 也不成立**——物体**全程躺在桌上**,对它做面外旋转会穿桌;所以面外对称**在每个相位都被桌面+重力破坏**,不是接触才破坏。

**合起来:对"物体全程在桌"的任务(pull/push/pick/drawer,且只用 yaw),合法对称群在 approach/contact/manipulate 都是同一个 SE(2)。没有 phase-dependent 的群变化 → 没有 contextual 群退化 → baseline(C4 ⊂ SE(2))处处正确、处处无 extrinsic floor。这才是 null 结果最根本的原因,也说明 §0–§2 的"接触退化 escape floor"叙事对这些任务不成立。**

**contextual 群退化只在合法群真随相位变时才有意义**,要求**物体在某相位离开桌面/被空中重定向**,自由空间朝向自由、接触/插入才被夺走:
- **peg-in-hole / 插入装配**:自由接近(朝向自由)→ 插入被孔轴约束(SE(3)→绕轴)。真断裂。
- **空中 6-DoF 重定向 / pick-and-place(带抬起+重定向)**:抓起后物体在**空中**才有 SE(3) 那一段(new_idea 原例的隐含前提)。

→ **§0–§6 的网络侧叙事,前提是先换到"合法群随相位变"的任务(见 [[iclr_strategy]] 的任务选择 + `sym_manip_tasks_baselines.md`)。任务的相位对称结构不真变,后面整套 context→网络架构都是空中楼阁。**

---

## 2. context → 子网络 的具体映射(推荐配置)

> ⚠️ 下表在"合法群随相位变化"的任务(插孔/空中 6-DoF)上才成立;桌面任务见 §1b。
