...
> **⚠️ 2026-07-27 重大前提更正**:上段"接触/负载相位对称被破坏"对**当前桌面任务不成立**。物体全程在桌:绕 z 的旋转在**所有相位**都合法(接触也是),pitch/roll 在**所有相位**都非法(approach 也穿桌)——合法群**在每相位都是同一个 SE(2)**,没有相位退化,baseline 处处正确、处处无 floor(这才是 null 的根因)。**因此本策略的网络侧 escape-floor 叙事,前提是先换到"合法群随相位真变"的任务**(peg-in-hole 插入 / 空中 6-DoF 重定向 / pick-and-place 带抬起重定向)。详见 [[context_to_network_designs]] §1b 与任务调研 `sym_manip_tasks_baselines.md`。**先定任务,再谈架构。**
