...
from rgbd_sym.env.embodied.pomdp.ortho_sensor import OrthographicSensor
...
class CloseLoopEnv(BaseEnv):
  ...
  def _getObservation(self, action=None):
    ...
    if self.obs_type == 'pixel':
      ...
      heightmap = self.heightmap['depth']
      ...
      self.heightmap['gripper_mask'] = np.zeros(gripper_img.shape, dtype=bool)
      self.heightmap['gripper_mask'][gripper_img == 1] = True
      return self._isHolding(), None, self.heightmap
    else:
  ...
  def _getHeightmap(self, gripper_pos=None, gripper_rz=None):
    ...
    if self.view_type == 'render_center':
      ...
      cam_up_vector = [-1, 0, 0]
      self.sensor.setCamMatrix(gripper_pos, cam_up_vector, target_pos)
      heightmap_dict = self.sensor.getHeightmap2(self.heightmap_size)
      # pc = self.sensor.getPointCloud(self.heightmap_size)
