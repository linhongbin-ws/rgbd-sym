...
from rgbd_sym.env.embodied.pomdp.close_loop_env import CloseLoopEnv
...
class CloseLoopPomdpBlockPushingEnv(CloseLoopEnv):
  ...
  def _getHeightmap(self, gripper_pos=None, gripper_rz=None):
    ...
    heightmap['depth'][out[:, 0].reshape(-1), out[:, 1].reshape(-1)] += 0.02
    heightmap['goal_mask'] = np.zeros(heightmap['depth'].shape, dtype=bool)
    heightmap['goal_mask'][out[:, 0].reshape(-1), out[:, 1].reshape(-1)] = True
    return heightmap
